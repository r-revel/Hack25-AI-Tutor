...
*   **Фронтенд (на выбор):**
    ...
    *   **Вариант 2:** MiniUiVue (минималистичный интерфейс для отладки и проверки основных функций).
*   **Хранилище:**
...
1.  **Верхняя панель:** Список загруженных учебных материалов по темам, добавление нового материала, просмотри истории прохождения тестов.
2.  **Центральная панель:**
    *   История диалога (пузырьки сообщений: пользователь справа, AI-репетитор слева).
    *   В каждом ответе AI: кнопка "Показать источники", при нажатии появляется список документов-оснований.
    *   Внизу: текстовое поле для ввода вопроса + кнопка "Отправить", "Сгенерировать вопросы по теме".
...
1.  Клонировать репозиторий: `git clone ...`
2.  Перейти в корневой каталог, запустить команду docker compose up -d -build 
3.  Blazor клиент по адресу http://localhost:8090/
```
