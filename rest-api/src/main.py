...
def startup_event():
    ...
    try:
        # seed topics
        seed_topics_from_jsonl(db, "/app/Notebooks/cloud_ru_docs.jsonl")
...
def start_test(
    ...
    test_session = crud.create_test_session(db, topic_id=topic_id, user_id=current_user.id)
    print("START_TEST: created session", test_session.id, "user_id=", test_session.user_id, "topic_id=", test_session.topic_id)
    return test_session
...
def get_test_questions(
    ...
    print("GET_QUESTIONS:", "session_id=", session_id, "current_user.id=", current_user.id)
    return questions
